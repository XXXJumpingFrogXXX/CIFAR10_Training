...
# 保存模型路径
MODEL_FILEPATH = 'saved_models/KNN_cifar10.pvl'
...
def show_res(model):
    ...
    print("K-近邻Accuracy:", accuracy)


if __name__ == "__main__":
    warnings.filterwarnings(action='ignore')
    print("下面进行K-近邻实验结果的展示...")
    # 根据模型是否创建采取不同方法
    try:
        model = joblib.load(MODEL_FILEPATH)
        print("K-近邻模型已创建，直接载入...")
    except:
        model = create_model()
        print("K-近邻模型未创建，首先创建...")
    train_model(model)
    show_res(model)
