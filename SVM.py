...
# 保存模型路径
MODEL_FILEPATH = 'saved_models/SVM_cifar10.pvl'
# 最大迭代次数
MAX_ITER = 100
...
# 建立模型
def create_model():
    model = SVC(kernel="rbf", C=1.0, gamma=0.1, max_iter=MAX_ITER)
    return model
...
def show_res(model):
    ...
    print("支持向量机Accuracy:", accuracy)


if __name__ == "__main__":
    warnings.filterwarnings(action='ignore')
    print("下面进行支持向量机实验结果的展示...")
    # 根据模型是否创建采取不同方法
    try:
        model = joblib.load(MODEL_FILEPATH)
        print("支持向量机模型已创建，直接载入...")
    except:
        model = create_model()
        print("支持向量机模型未创建，首先创建...")
    train_model(model)
    show_res(model)
