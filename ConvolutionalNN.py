...
# 保存模型路径
MODEL_FILEPATH = 'saved_models/CNN_cifar10.h5'
# 训练epoch
TRAIN_EPOCH = 200
# 批大小
BATCH_SIZE = 256
...
def train_model(model):
    ...
    train_history = model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=TRAIN_EPOCH, batch_size=BATCH_SIZE, verbose=1)
    # 模型保存
...
def show_res(model):
    ...
    print('卷积神经网络Loss:', score[0])
    print('卷积神经网络Accuracy:', score[1])
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("卷积神经网络模型已创建，直接加载...")
    except:
        model = create_model()
        print("卷积神经网络模型未创建，首先创建...")
    # 训练模型并获取训练历史
